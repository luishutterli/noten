...
function GradeListTable({ averagedGrades, subjects, showGrouped, groupings, showRounded }) {
    ...
    const round = useCallback((value, decimals) => {
        return Math.round(value * 10 ** decimals) / 10 ** decimals;
    }, []);

    const handleMouseEnter = useCallback(
        (event, grade, isShownRounded) => {
            if (!isShownRounded) return;
            if (Number.isNaN(grade)) return;
            setTooltip({
                visible: true,
                content: `Note: ${round(grade, 3).toFixed(3)}`,
                position: { x: event.clientX, y: event.clientY - 30 + window.scrollY },
            });
        },
        [round],
    );
    ...
    return (
        <div>
            {!showGrouped ? (
                <table className="w-full table-auto border-collapse shadow-lg bg-white">
                    ...
                    <tbody>
                        {subjects.map((subject, index) => {
                            ...
                            const displayedGrade = Number.isNaN(grade) ? "-" : showRounded ? mround(grade, 0.5) : round(grade, 2);
                            const subjectName = subject.teacher ? `${subject.name} (${subject.teacher})` : subject.name;
                            ...
                            return (
                                <tr
                                    ...
                                    onMouseEnter={(e) => handleMouseEnter(e, grade, showRounded)}
                                    onMouseLeave={handleMouseLeave}>
                                    <td className="border p-2">{subjectName}</td>
                                    <td className={`border p-2 w-16 ${gradeClass}`}>{displayedGrade}</td>
                                </tr>
                            );
                        })}
                    </tbody>
                ...
                <>
                    {groupings.map((grouping) => {
                        return (
                            <table key={grouping.name} className="w-full table-auto border-collapse shadow-lg bg-white">
                                <thead className="bg-gray-200">
                                    <tr>
                                        <th className="text-start border p-2">{grouping.name}</th>
                                        <th className="text-start border p-2 w-16">Note</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {grouping.members.map((member) => {
                                        ...
                                        return (
                                            <tr
                                                ...
                                                onMouseLeave={handleMouseLeave}>
                                                <td className="border p-2">{subjectName}</td>
                                                <td className={`border p-2 ${gradeClass}`}>{displayedGrade}</td>
                                            </tr>
                                        );
                                    })}
                                </tbody>
            ...
            <div className="mt-4 p-2 bg-gray-100 border rounded">
                ...
                <p className="text-xl">
                    {(() => {
                        const halfterm = subjects.find((s) => s.type === "halfterm");
                        ...
                        if (isIMS) {
                            const informatik = subjects.find((s) => s.name === "Informatik" && s.type === "group");
                            const iGrade = averagedGrades[informatik.id];
                            const renderedGrade = iGrade ? `${mround(iGrade, 0.5)} (${iGrade.toFixed(2)})` : "-";
                            return (
                                <>
                                    Promotionsschnitt: {mround(grade, 0.5)} ({grade.toFixed(2)})<br />
...
GradeListTable.propTypes = {
    ...
    showRounded: PropTypes.bool.isRequired,
};
